...
def collect_and_analyze_books():
    ...
    for topic in topics:
        ...
        result = fetch_books_by_topic(topic)
        ...
        for book in books:
            ...
            pub_year = int(book.get('pubDate').split('-')[0])
            prices.append(price)
    ...
    topic_stats[topic]
    for topic in topics:
        overall_price += topic_stats[topic].get('avg_price')
        overall_year += topic_stats[topic].get('avg_pub_year')
    overall_avg_price = overall_price / len(topics)
    overall_avg_year = overall_year // len(topics)
