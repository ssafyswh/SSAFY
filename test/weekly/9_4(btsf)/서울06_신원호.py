...
# 경로 탐색 알고리즘
def dijkstra(grid, start, target, wall):
    rows, cols = len(grid), len(grid[0])
    hq = [(0, -int(my_allies['M'][2]), -int(my_allies['M'][3]), start, [])]
    visited = {start}

    while hq:
        turn, fire, mega, (r, c), actions = heappop(hq)
        for d, (dr, dc) in enumerate(DIRS):
            for multi in range(1, 4):
                ...
                if 0 <= nr < rows and 0 <= nc < cols:
                    if grid[nr][nc] == 'R':
                        break
                    if (nr, nc) == target or grid[nr][nc][0] == 'E':
                        if mega:
                            return actions + [FIRE_MEGA_CMDS[d]]
                        elif fire:
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] not in wall and (nr, nc) not in visited:
                if grid[nr][nc][0] == 'M':
                    continue
                if grid[nr][nc] == 'T' and not fire and not mega:
                    continue
                if mega == 0 and codes:
                    heappush(hq, (turn + 1, fire, mega + 1, (dr, dc), actions + [f'G {decoding(codes[0])}']))
                    continue
                visited.add((nr, nc))
                if grid[nr][nc] == 'T':
                    if fire:
                        heappush(hq, (turn + 2, fire + 1, mega, (nr, nc), actions + [FIRE_CMDS[d]] + ['A']))
                    else:
                        heappush(hq, (turn + 2, fire, mega + 1, (nr, nc), actions + [FIRE_MEGA_CMDS[d]] + ['A']))
                else:
                    heappush(hq, (turn + 1, fire, mega, (nr, nc), actions + [MOVE_CMDS[d]]))
...
# 최초 경로 탐색
actions = dijkstra(map_data, start, target, CANNOT_PATH_SYMBOL)
...
while game_data is not None:
    ...
    if not actions:
        start, target = find_positions(map_data, START_SYMBOL, TARGET_SYMBOL)
        actions = dijkstra(map_data, start, target, CANNOT_PATH_SYMBOL) if start and target else []
